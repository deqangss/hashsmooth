...
class DQN(object):
    ...
    def learn(self, MEMORY_CAPACITY, BATCH_SIZE, N_STATES, TARGET_REPLACE_ITER=10, GAMMA=0.9):
        ...
        b_state = torch.FloatTensor(b_memory[:, :N_STATES]).to(self.device) # N_States : state dimension = degree dimension
        ...
        q_target = GAMMA * q_next.max(1)[0].view(BATCH_SIZE, 1)  # shape (batch, 1)
