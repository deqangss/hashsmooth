...
class HashSmooth4MalScan(HashSmooth):
    ...
    def certify(self, x: np.ndarray, label: int, adj_size: int, n_selection: int, n_estimation: int, alpha: float,
        ...
        counts_selection, k_subhashcodes = self.sample_lsh_funcs_a_point(malscan_feature_vec,
                                                                         n_selection,
    ...
    def sample_lsh_funcs_a_point(self, x, n: int, batch_size=64, n_subfeatures=[], k_hashcode=16,
                                 top_k=1, device='cpu', verbose=False):
        assert n > 0
        self.base_classifier.eval()
        if len(n_subfeatures) == 0:
            n_subfeatures = [x.shape[0]]
