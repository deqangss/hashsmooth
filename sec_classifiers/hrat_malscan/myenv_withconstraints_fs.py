...
from sec_classifiers.hrat_malscan.randomsmooth_hrat_malscan_det_fs import RandomSmooth4MalScan
...
class CFGModifierEnvConstraints(object):
    ...
    def __init__(self, target_graph, tar_label,
    ...
    def get_loss(self, feature):
        ...
        if isinstance(self.malware_detector, MalScan):
            ...
            for idx in range(n_counts // self.batch_size + 1):
                ...
                print(feature_tran.shape, dist.shape)
                # dist = torch.sum((torch.squeeze(X_train.to(self.device)) - torch.squeeze(feature.float())).pow(2.), 1)
