...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # max_length=sum([i!=1 for i in self.examples[item].position_idx])
        max_length = sum([i == 0 for i in self.examples[item].position_idx])
        # sequence can attend to sequence
        attn_mask[:node_index, :node_index] = True
        # special tokens attend to all tokens
        for idx, i in enumerate(self.examples[item].input_ids):
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            inputs_ids_mask = model.transform(inputs_ids, tokenizer)
            loss,logits = model(inputs_ids_mask,attn_mask,position_idx,labels)
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    recall = recall_score(y_trues, preds, average='macro')
    from sklearn.metrics import precision_score
    precision = precision_score(y_trues, preds, average='macro')
    from sklearn.metrics import f1_score
    f1 = f1_score(y_trues, preds, average='macro')
    result = {
...
def test(args, model, tokenizer):
    ...
    count1 = preds_onehot[range(preds.shape[0]), top2[:, 0]].astype(int)
    count2 = preds_onehot[range(preds.shape[0]), top2[:, 1]].astype(int)
    ...
    # accuracy, b_accuracy, fnr, fpr, f1 = measurement(y_trues, preds)
    # logger.info(
    #     "Model of {} achieves the accuracy: {:.6f}%, balanced accuracy: {:.6f}%".format(args.model, accuracy * 100,
    #                                                                                     b_accuracy * 100))
    # MSG = "False Negative Rate (FNR) is {:.5f}%, False Positive Rate (FPR) is {:.5f}%, F1 score is {:.5f}%"
    # logger.info(MSG.format(fnr * 100, fpr * 100, f1 * 100))

    y_trues = y_trues[~abstain_flag]
    y_preds = preds[~abstain_flag]
    logger.info('Filter out outlier: {}.'.format(np.sum(abstain_flag)))
    acc2 = accuracy_score(y_trues, y_preds)
    cert_acc = np.sum(y_trues == y_preds) / float(len(preds))
    ...
    result.update({
        "test_acc_filter": float(acc2),
        "test_acc_cert": float(cert_acc),
...
def certify(args, model, tokenizer, prefix="",pool=None,threshold=0.5):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
        ...
        position_idx = batch[2].to(args.device)
        label = batch[3].to(args.device)
        bs, dim = inputs_ids.shape
        with torch.no_grad():
            logits = []
            for sample_idx in range(args.n_sampling):
                inputs_ids_mask = model.transform(inputs_ids, tokenizer)
                lm_loss, logit = model(inputs_ids_mask, attn_mask, position_idx, label)
                logits.append(logit.cpu().numpy())
            logits = np.concatenate(logits, 0)
            ...
            preds_estimation = np.eye(args.number_labels)[preds.astype(int)].sum(axis=-2)

            n_targeted = preds_estimation[range(top2.shape[0]), top2[:, 0].astype(int)]
            ...
            radius = np.zeros_like(label.shape, dtype=object)
