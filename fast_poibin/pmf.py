...
if sys.version_info[0:2] <= (3, 8):
    FloatSequence = Union[Sequence[float], ]
else:
    FloatSequence = Union[Sequence[float], ]


def calc_pmf(probabilities: FloatSequence, dp_step: int = DP_STEP):
    ...
    if size < dp_step * 2:
        res = calc_pmf_dp(np.array(probabilities, np.float64))
        return res
    if dp_step > 0:
