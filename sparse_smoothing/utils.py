...
def sample_multiple_graphs(attr_idx, edge_idx, sample_config, n, d, nsamples):
    ...
    # dense_atta_per = torch.sparse.FloatTensor(per_attr_idx.cpu(), torch.ones(per_attr_idx.shape[1]), torch.Size([n,d])).to_dense()
    # dense_atta = torch.sparse.FloatTensor(attr_idx.cpu(), torch.ones(attr_idx.shape[1]),
    #                                           torch.Size([n, d])).to_dense()
    # print(torch.sum((dense_atta_per - dense_atta) > 0), (torch.sum(dense_atta_per - dense_atta < 0)),
    #       attr_idx.shape[1] * pf_minus_att, (np.cumprod(dense_atta.shape)[-1] - attr_idx.shape[1]) * pf_plus_att)
    # exit(1)
    ...
    dense_edge_per = torch.sparse.FloatTensor(per_edge_idx.cpu(), torch.ones(per_edge_idx.shape[1]), torch.Size([n,n])).to_dense()
    dense_edge = torch.sparse.FloatTensor(edge_idx.cpu(), torch.ones(edge_idx.shape[1]),
                                              torch.Size([n, d])).to_dense()
    # print(pf_minus_adj, pf_plus_adj)
    # print(torch.sum((dense_edge_per - dense_edge) > 0), (torch.sum(dense_edge_per - dense_edge < 0)),
    #       attr_idx.shape[1] * pf_minus_att, (np.cumprod(dense_edge.shape)[-1] - attr_idx.shape[1]) * pf_plus_att)
    # exit(1)
...
def get_mnist_dataloaders(batch_size, random_seed=0, num_workers=-1, pin_memory=False, root='../dataset_cache', shuffle=True):
    ...
    dataset_test_bin = TensorDataset(x_test_bin[:100], dataset_test.targets[:100])
